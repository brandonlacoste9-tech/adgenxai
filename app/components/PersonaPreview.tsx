    ...
    <section id="personas" className="bg-white">
      <div className="max-w-7xl mx-auto px-6 py-12 md:py-16">
        <div className="mb-6 flex items-end justify-between gap-4">
          ...
          <a 
            href="#create" 
            className="hidden sm:inline-block rounded-xl bg-[#35E3FF] text-[#071022] font-semibold px-5 py-2.5 
                       hover:bg-[#29c7e1] hover:scale-105 active:scale-95
                       shadow-[0_6px_30px_rgba(53,227,255,.35)] 
                       hover:shadow-[0_8px_40px_rgba(53,227,255,.45)]
          ...
          {PERSONAS.map((p) => (
            <article 
              ...
              className={`rounded-2xl border border-black/10 bg-white p-5 
                         ...
                         ${activePersona === p.name ? 'ring-2 ring-[#35E3FF] shadow-lg' : ''}`}
              onClick={() => setActivePersona(activePersona === p.name ? null : p.name)}
              onMouseEnter={() => setActivePersona(p.name)}
              onMouseLeave={() => setActivePersona(null)}
              role="button"
              tabIndex={0}
              aria-label={`${p.name} persona - ${p.tone} - ${p.accent}`}
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  setActivePersona(activePersona === p.name ? null : p.name);
                }
              }}
            >
              ...
              <div className="mt-4 flex items-center gap-2">
                {["TikTok","Instagram","YouTube","X"].map((n) => (
                  <span 
                    key={n} 
                    className="text-xs rounded-lg border border-black/10 bg-white px-2.5 py-1 
                               hover:bg-[#35E3FF]/10 transition-colors"
                  >
                    {n}
                  </span>
                ))}
                <a 
                  href="#create" 
                  className="ml-auto text-sm rounded-lg bg-[#071022] text-white px-3 py-1.5 
                             hover:bg-[#35E3FF] hover:text-[#071022] 
                             active:scale-95
                             transition-all duration-200"
                  onClick={(e) => e.stopPropagation()}
                  aria-label={`Use ${p.name} persona`}
                >
                  Use
                </a>
              </div>
